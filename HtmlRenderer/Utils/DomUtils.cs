...
namespace HtmlRenderer.Utils
{
    ...
    internal sealed class DomUtils
    {
        ...
        public static CssBox GetLinkBox(CssBox box, Point location)
        {
            if(box != null)
            {
                if(box.HtmlTag != null && box.HtmlTag.Name.Equals("a",StringComparison.OrdinalIgnoreCase))
                {
        ...
        /// <param name="location">the location to find the box at</param>
        /// <returns>css word box if exists or null</returns>
        public static CssBoxWord GetCssBoxWord(CssBox box, Point location)
        {
            if (box != null)
            {
                if (box.LineBoxes.Count > 0)
        ...
        private static int GetSelectedPlainText(StringBuilder sb, CssBox box)
        {
            ...
            if (sb.Length > 0)
            {
                ...
                // paragraphs has aditional newline for nice formatting
        ...
        private static HashSet<HtmlTag> CollectSelectedHtmlTags(CssBox root)
        {
            var selectedTags = new HashSet<HtmlTag>();
            var maybeTags = new HashSet<HtmlTag>();
        ...
        private static bool CollectSelectedHtmlTags(CssBox box, HashSet<HtmlTag> selectedTags, HashSet<HtmlTag> maybeTags)
        {
            ...
            foreach (var word in box.Words)
            {
                if (word.Selected)
                {
                    if (box.HtmlTag != null)
                    {
                        selectedTags.Add(box.HtmlTag);
                    }
                    selectedTags.UnionWith(maybeTags);
                    maybeTags.Clear();
                    isInSelection = true;
                }
            }
