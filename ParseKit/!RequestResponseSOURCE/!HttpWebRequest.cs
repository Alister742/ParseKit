...
{ 
    ...
    public class HttpWebRequest : WebRequest, ISerializable {
        ...
        private int                     _NestedWriteSideCheck; //To keep track nested responses for [....] case 
        ...
        */
        ...
        /*
        // Consider removing. 
        //the headers should already be set at this point
        ...
        */
        ...
        internal void SetRequestContinue(CoreResponseData continueResponse) 
        {
            ...
            { 
                // Generally, in [....] mode there will not be a timer (instead PollAndRead times out).  However, in mixed mode
        ...
        internal void CheckWriteSideResponseProcessing() 
        {
            ...
            { 
                GlobalLog.Print("HttpWebRequest#" + ValidationHelper.HashString(this) + "::CheckWriteSideResponseProcessing() - responseData = DBNull means no response yet, " +  (Async"async: closing write side window": "[....]: leaving write side window opened")); 
                return;
            } 
            ...
            { 
                GlobalLog.Print("HttpWebRequest#" + ValidationHelper.HashString(this) + "::CheckWriteSideResponseProcessing() - Repeated [....] check _NestedWriteSideCheck = " + _NestedWriteSideCheck + ", response =" + responseData.GetType().FullName);
                return;
            }
        ...
        internal void SetAndOrProcessResponse(object responseOrException)
        {
            ...
            if (!Async)
                throw new InternalException(); //Consider making an Assert later. If the condition is met it _might_ process a [....] request on non submitting thread.
        ...
        { 
            ...
            ContextAwareResult context =
                (HttpWriteMode == HttpWriteMode.None || _OldSubmitWriteStream != null || _WriteAResult == null  
        ...
        { 
            if (!Async)
            { 
                GlobalLog.ThreadContract(ThreadKinds.User | ThreadKinds.[....], "HttpWebRequest#" + ValidationHelper.HashString(this) + "::GetWritingContext"); 
                return null;
            } 

			// _WriteARequest == null is the case where the user specifies POST but doesn't call GetRequestStream().
            ContextAwareResult context =  (HttpWriteMode == HttpWriteMode.None || HttpWriteMode == HttpWriteMode.Buffer ||
                m_PendingReturnResult == DBNull.Value || m_OriginallyBuffered || _WriteAResult == null  
                _ReadAResult : _WriteAResult) as ContextAwareResult;
            if (context == null) 
            { 
        ...
        internal override ContextAwareResult GetReadingContext() 
        {
            if (!Async) 
            {
                GlobalLog.ThreadContract(ThreadKinds.User | ThreadKinds.[....], "HttpWebRequest#" + ValidationHelper.HashString(this) + "::GetReadingContext");
                return null;
            } 
                ...
                if ((ContentLength>0 || HttpWriteMode==HttpWriteMode.Chunked) && ExpectContinue && _ServicePoint.Understands100Continue) { 
                    if (async  m_ContinueGate.StartTrigger(true) : m_ContinueGate.Trigger(true))
                    {
                        ...
                        { 
                            ...
                            // On the [....] Path, we need to poll the Connection to see if there is any Data 
                ...
                { 
                    ...
                    if (AllowWriteStreamBuffering) {
                        ...
                        if (_OldSubmitWriteStream != null)
                        {
                            ...
                            // 
    #if DEBUG
                            using (GlobalLog.SetThreadKind(ThreadKinds.[....])) { 
    #endif
        ...
        private int GenerateConnectRequestLine(int headersSize) {
            ...
            HostHeaderString host = new HostHeaderString(HostAndPort(true)); 
        ...
        internal string HostAndPort(bool addDefaultPort) {
            ...
            String HostToUse;
            if (UriToUse.HostNameType == UriHostNameType.IPv6) { 
                // if IPv6 literal then remove any scope ID & add brackets
                HostToUse = "[" + TrimScopeID(UriToUse.DnsSafeHost) + "]"; 
            } else { 
                HostToUse = UriToUse.DnsSafeHost;
            } 
            if (addDefaultPort || !UriToUse.IsDefaultPort) {
                return HostToUse + ":" + UriToUse.Port;
            } else {
                return HostToUse; 
            }
        } 
        ...
        private int GenerateProxyRequestLine(int headersSize) {
            ...
            HostHeaderString host = new HostHeaderString(HostAndPort(false));
        ...
        internal void UpdateHeaders() {
            ...
            HostHeaderString hhs = new HostHeaderString(HostAndPort(false)); 
        ...
        internal string GetConnectionGroupLine() { 
            ...
            if ((_Uri.Scheme == Uri.UriSchemeHttps) || IsTunnelRequest) {
                if (UsesProxy) {
                    connectionLine.Append(HostAndPort(true)); 
            ...
            { 
                    ...
                    ResponseStatusCode==HttpStatusCode.RedirectKeepVerb ))   // 307
                {
                    ...
                    try {
                        ...
                        ExecutionContext x = Async  GetReadingContext().ContextCopy : null;
