            ...
            using (new MultithreadSafeCallScope()) 
            {
                ...
                if (!asyncResult.IsCompleted) {
                    ...
                    if (SafeNativeMethods.GetWindowThreadProcessId(new HandleRef(marshaler, marshaler.Handle), out pid) == SafeNativeMethods.GetCurrentThreadId()) { 
                        ...
                        WaitForWaitHandle(asyncResult.AsyncWaitHandle);
                    }
                }
        ...
        private void WmEraseBkgnd(ref Message m) { 
            if (GetStyle(ControlStyles.UserPaint)) { 
                ...
                if (!(GetStyle(ControlStyles.AllPaintingInWmPaint)) ) {
                    ...
                    PaintEventArgs pevent = new PaintEventArgs(dc, Rectangle.FromLTRB(rc.left, rc.top, rc.right, rc.bottom)); 
        ...
        private void WmPaint(ref Message m) {
            ...
            if (doubleBuffered) {
                ...
                try {
                    if (clip.Width > 0 && clip.Height > 0) { 
                        ...
                        if (BufferPinkRect.Enabled) { 
                            using (BufferedGraphics bufferedGraphics = BufferContext.Allocate(dc, band)) {
                                bufferedGraphics.Graphics.FillRectangle(new SolidBrush(Color.Red), band); 
                        ...
                        { 
                            ...
                            using (PaintEventArgs e = new PaintEventArgs(g, clip)) { 
                                PaintWithErrorHandling(e, PaintLayerBackground, /* dispose pevent */ false);
                                g.Restore(state);
                ...
                if (m.WParam == IntPtr.Zero) {
                    NativeMethods.PAINTSTRUCT ps = new NativeMethods.PAINTSTRUCT(); 
                    IntPtr hWnd = this.Handle;
                    IntPtr dc = UnsafeNativeMethods.BeginPaint(new HandleRef(this, hWnd), ref ps); 
                    IntPtr oldPal = SetUpPalette(dc, false, false); 
                    try {
                        PaintEventArgs pevent = new PaintEventArgs(dc, new Rectangle(ps.rcPaint_left, ps.rcPaint_top, 
                                                                                     ps.rcPaint_right - ps.rcPaint_left,
                                                                                     ps.rcPaint_bottom - ps.rcPaint_top));
                        try {
                            if (GetStyle(ControlStyles.AllPaintingInWmPaint) /*|| GetStyle(ControlStyles.OptimizedDoubleBuffer)*/) { 
                            ...
                            // VSWhidbey 261657 -- if something really bad happens, or if Close was called, this window
                            // may be disposed now.  If not, make sure we've got the right hWnd. 
                    ...
                    PaintEventArgs pevent = new PaintEventArgs(m.WParam, ClientRectangle);
