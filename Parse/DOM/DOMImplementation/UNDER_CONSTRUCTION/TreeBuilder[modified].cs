...
namespace Parse.DOM
{
    public abstract class TreeBuilder<T> : ITreeBuilder where T : class
    {
        public delegate void NotifyDel(string msg);
         public delegate void StartTagDel (ElementName eltName, HtmlAttributes attributes, bool selfClosing);
        public delegate void EndTagDel(ElementName eltName);
        public delegate void TokenDataDel(string text);
        public event NotifyDel OnWarring;
        public event NotifyDel OnError;
        public event NotifyDel OnFatal;
        public event EventHandler OnStart;
        public event EventHandler OnEnd;
        public event StartTagDel OnStartTag;
        public event EndTagDel OnEndTag;
        public event TokenDataDel OnTokenData;
        ...
        protected void Fatal()
        {
            if (OnFatal!=null)
            {
                OnFatal("TreeBuilder fatal, tokenizerLine " + tokenizer.LineNumber);
            }
        }

        #region Error
        private void Err(string p)
        {
        ...
        private void ReportUnclosedElementNameAndLocation(int pos)
        {
            ...
            Err("Unclosed element \u201C" + node.popName + "\u201D.");
        }
        ...
        #region ITreeBuilder_IMPLEMENTATION
        /// <summary>
        ...
        public void StartTokenization(Tokenizer self)
        {
            ...
            if (fragment)
            {
                ...
                {
                    tokenizer.SetStateAndEndTagExpectation(Tokenizer.SCRIPT_DATA, contextName);
                }
                else
                {
        ...
        public void StartTag(ElementName elementName, HtmlAttributes attributes, bool selfClosing)
        {
            if (OnStartTag != null)
            {
                OnStartTag(elementName, attributes, selfClosing);
            }
            ...
            for (; ; )
            {
                ...
                switch (mode)
                {
                    ...
                    case InsertionMode.FRAMESET_OK:
                        switch (group)
                        {
                            ...
                            case DispatchGroup.SELECT:
                                if (mode == InsertionMode.FRAMESET_OK
                                        && !(group == DispatchGroup.INPUT && "hidden".NoncaseEqual(attributes.GetValue(AttributeName.TYPE))))
                                {
                                    framesetOk = false;
                                ...
                                case DispatchGroup.DD_OR_DT:
                                    ...
                                    for (; ; )
                                    {
                                        ...
                                        { // LI or
                                            ...
                                            if (OnError != null                                                    && eltPos != currentPtr)
                                            {
                    ...
                    case InsertionMode.INITIAL:
                         ...
                         */
                        DocumentModeInternal(DocumentMode.QuirksMode, null, null, false);
                        /*
                         * Then, switch to the root element mode of the tree
                         * construction stage
                         */
                    ...
                    case InsertionMode.BEFORE_HEAD:                                 //(+)script part
                        switch (group)
                        {
                            case DispatchGroup.SCRIPT:
                    ...
                    case InsertionMode.IN_BODY:
                        switch (group)
                        {
                            ...
                            case DispatchGroup.P:
                                ...
                                Debug.Assert(eltPos != NOT_FOUND_ON_STACK);
                                if (eltPos != currentPtr)
                                {
                                    ErrUnclosedElements(eltPos, name);
                                }
...
breakEndtagloop:
        ...
        public void Comment(char[] buf, int start, int length)
        {
            if (OnTokenData!=null)
            {
                OnTokenData(new string(buf, start, length));
            }
                            ...
                            default:
                                ...
                                switch (mode)
                                {
                                    ...
                                    case InsertionMode.BEFORE_HEAD:
                                         ...
                                         */
                                        FlushCharacters();
                                        AppendToCurrentNodeAndPushHeadElement(new HtmlAttributes((int)InsertionMode.IN_HEAD));
                                        mode = InsertionMode.IN_HEAD;
                                        /*
                                         * then reprocess the current token.
                                         * 
        ...
        private void CheckMetaCharset(HtmlAttributes attributes)
        {
            ...
            if (!"content-type".NoncaseEqual(attributes.GetValue(AttributeName.HTTP_EQUIV)))
            {
                return;
            }
        ...
        private void AppendToCurrentNodeAndPushElementMayFoster(ElementName elementName, HtmlAttributes attributes, T form)
        {
            ...
            T elt = CreateElement("http://www.w3.org/1999/xhtml", elementName.name, attributes, fragment ? null : form);
            StackNode<T> current = stack[currentPtr];
            if (current.IsFosterParenting)
            {
                Fatal();
        ...
        protected abstract void MarkMalformedIfScript(T elt);
